...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		ImGui::DragFloat3("sacles[0]", &scales[0].x, 0.01f);
		ImGui::DragFloat3("translates[1]", &translates[1].x, 0.01f);
		ImGui::DragFloat3("rotates[1]", &rotates[1].x, 0.01f);
		ImGui::DragFloat3("sacles[1]", &scales[1].x, 0.01f); 
		ImGui::DragFloat3("translates[2]", &translates[2].x, 0.01f);
		ImGui::DragFloat3("rotates[2]", &rotates[2].x, 0.01f);
		ImGui::DragFloat3("sacles[2]", &scales[2].x, 0.01f);
		ImGui::End();
		...
		//肩
		Matrix4x4 sholderTransformMatrix = Matrix4x4::MakeAffineMatrix(scales[0], rotates[0], translates[0]);
		Sphere shoulder = { Vector3(sholderTransformMatrix.m[3][0],sholderTransformMatrix.m[3][1],sholderTransformMatrix.m[3][2]), 0.1f};
		//肘
		Matrix4x4 elbowTransformMatrix = Matrix4x4::MakeAffineMatrix(scales[1], rotates[1], translates[1])* sholderTransformMatrix;
		Sphere elbow = { Vector3(elbowTransformMatrix.m[3][0], elbowTransformMatrix.m[3][1], elbowTransformMatrix.m[3][2]), 0.1f };
		//手
		Matrix4x4 handTransformMatrix = Matrix4x4::MakeAffineMatrix(scales[2], rotates[2], translates[2])* elbowTransformMatrix;
		Sphere hand = { Vector3(handTransformMatrix.m[3][0], handTransformMatrix.m[3][1], handTransformMatrix.m[3][2]), 0.1f };
		
		Matrix4x4 sholderWvpMatrix = sholderTransformMatrix * ViewProjectionMatrix;
		Matrix4x4 elbowWvpMatrix = elbowTransformMatrix * ViewProjectionMatrix;
		Matrix4x4 handWvpMatrix = handTransformMatrix * ViewProjectionMatrix;
		Vector3  sholderScreen = Matrix4x4::ScreenTransform(Vector3{}, sholderWvpMatrix, viewportMatrix);
		Vector3 elbowScreen = Matrix4x4::ScreenTransform(Vector3{}, elbowWvpMatrix, viewportMatrix);
		Vector3  handScreen = Matrix4x4::ScreenTransform(Vector3{}, handWvpMatrix, viewportMatrix);
		///
		/// ↑更新処理ここまで
		///
		...
		Novice::DrawLine(int(sholderScreen.x), int(sholderScreen.y), int(elbowScreen.x), int(elbowScreen.y), WHITE);
